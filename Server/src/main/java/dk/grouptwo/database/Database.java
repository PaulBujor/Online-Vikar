...
public class Database implements Persistence {
    ...
    @Override
    public ArrayList<Job> getAllJobHistoryEmployerFromDB(
            Employer employer) {
        ArrayList<Job> jobs = new ArrayList<>();
    ...
    @Override
    public ArrayList<Job> getUpcomingJobsWorkerFromDB(Worker worker) {

        ArrayList<Job> jobs = new ArrayList<>();
        String SQL = "SELECT * FROM job INNER JOIN works ON job.jobID = works.jobID WHERE works.cpr=? AND job.status='pending'";
    ...
    @Override
    public ArrayList<Worker> getAllAppliedWorkers(Job job) {
    ...
    @Override
    public ArrayList<Worker> getAllAcceptedWorkers(Job job) {
        ArrayList<Worker> workers = new ArrayList<>();
    ...
    public ArrayList<License> getAllLicencesByCPR(String cpr) {
        ...
        Connection conn = null;
        try {
            String SQL = "SELECT * FROM license where cpr=?";
            conn = DatabaseConnection.getInstance().connect();
            pstm = conn.prepareStatement(SQL);
            pstm.setString(1, cpr);
            rs = pstm.executeQuery();
    ...
    private void process(ResultSet rs, Job job) throws SQLException {
        ...
        job.setWorkersNeeded(rs.getInt("workersneeded"));
